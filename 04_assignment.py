...
class PandasChain:
    ...
    def __commit_block(self,block): 
        ...
        self.__block_hash = hashlib.sha256(str(str(self.__prev_hash)+str(self.__id)+str(dt.datetime.now())+str(self.__seq_id)+str(np.random.randint(low=0, high=99))+str(block.get_simple_merkle_root())).encode('utf-8')).hexdigest() # Create block hash
    ...
    def get_values(self):
        ...
        # Loop through committed blocks to get timestamp and values.
        for committed_block in self.__chain:
            committed_block_timestamp, committed_block_value = committed_block.get_values()
            for committed_timestamp in committed_block_timestamp:
                timestamp.append(committed_timestamp)
            for committed_value in committed_block_value:
                value.append(committed_value)
            
        # Loop through uncommitted block to get timestamp and values.
        current_block_timestamp, current_block_value = self.__current_block.get_values()
        for current_timestamp in current_block_timestamp:
            timestamp.append(current_timestamp)
        for current_value in current_block_value:
    ...
    # and plot them. Use an incrementing sequential number for x and the transaction values for y.
    # Extra Credit (+10 points): Have get_values() return the timestamp for each value transacted and plot a timeseries as well.
    def __plot_transactions(self):

        get_timestamp_values = self.get_values()
        time = get_timestamp_values[0]
        coin = get_timestamp_values[1]

        df_dict = {'Timestamp':time, 'Value': coin}
        df = pd.DataFrame(df_dict)
        df['Sequential Number'] = df.index + 1

        # Plot for incrementing sequential number and transaction values.
        plt.plot(df['Sequential Number'],df['Value'])
        plt.xlabel('Sequential Number')
        plt.ylabel('Coins')
        plt.title('Plot of Sequential Number vs Coin Transactions')
        plt.show()

        # Plot for timeseries and transaction values.
        plt.plot(df['Timestamp'],df['Value'])
        plt.xlabel('Timestamp')
        plt.ylabel('Coins')
        plt.title('Plot of Timestamp vs Coin Transactions')
        plt.show()
...
class Block:
    ...
    def display_header(self): 
        print("Sequence ID: {0}, Status: {1}, Block Hash: {2}, Previous Block's Hash: {3}, Merkel Hash: {4}, Number of Transactions in the Block: {5}".format(self.__seq_id, self.__status, self.__block_hash, self.__prev_hash, self.__merkle_tx_hash, len(self.__transactions)))
    ...
    def add_transaction(self,s,r,v):
        ...
        new_transaction = {'Timestamp':ts, 'Sender':s, 'Receiver':r, 'Value':v, 'TxHash':tx_hash} # Create DataFrame with transaction data (a DataFrame with only 1 row)
        self.__transactions = self.__transactions.append(new_transaction, ignore_index=True) # Append to the transactions data
        
    # 10 pts -Print all transactions contained by this block
    def display_transactions(self): 
        print('All Transactions in this Block', self.__transactions, sep='\n')
    ...
    def get_simple_merkle_root(self):
        self.__merkle_tx_hash = hashlib.sha256((self.__transactions['TxHash'].str.cat()).encode('utf-8')).hexdigest()
        return self.__merkle_tx_hash
    
    # Return timestamp and transaction values.
    def get_values(self):
        return([self.__transactions["Timestamp"],self.__transactions["Value"]])
